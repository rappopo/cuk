...
module.exports = function(cuk, trace) {
  ...
  fs.ensureDirSync(path.join(cuk.dir.root, 'cuks', 'core', 'helper'))

  cuk.lib.helper = name => {
    let names = (name || '').split(':')
    if (names.length === 1) names = ['root', names[0]]
  ...
  const action = opt => {
    opt.pkg.cuks.core.helper[opt.key] = opt.value
    trace('Helper » %s:%s', opt.pkg.id, opt.key)
  }
